...
public final class KcalUtils {
    ...
    private static final String[] PROFILE_SETTINGS = {"red", "green", "blue", "saturation", "contrast"};
    ...
    public static void setColorProfile(int profileIndex, SharedPreferences sharedPrefs) {
        try {
            ...
            if (profileSettings.length != PROFILE_SETTINGS.length) {
                return;
            }

            for (int i = 0; i < profileSettings.length; i++) {
                if (profileSettings[i] != -1) {
                    sharedPrefs.edit().putInt(PROFILE_SETTINGS[i] + "_slider", profileSettings[i]).apply();
                }
            }
