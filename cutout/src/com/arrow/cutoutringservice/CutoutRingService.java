...
public class CutoutRingService extends BroadcastReceiver {
    ...
    private void adjustParamsToOrientation() {
        final int orientation = getOrientation();
        int gravity = Gravity.CENTER;
        int x = X_OFFSET;
        mVisibility = VISIBLE;

        Log.i(TAG, String.format("Adjusting to orientation %d", orientation));
        ...
        switch (orientation) {
            ...
            case ROTATION_270:
                ...
                break;
            default:
                Log.w(TAG, String.format("Unknown orientation %d, hiding the view", orientation));
    ...
    private void setVisibility(boolean shown) {
        Handler.getMain().postDelayed(() ->
            mRingView.setVisibility(shown ? mVisibility : INVISIBLE), shown ? 0 : ANIMATION_MS);
